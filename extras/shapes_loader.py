...
class ShapeDataset(torch.utils.data.Dataset):
    ...
    def get_gt(self):
        ...
        categories = [{"id": 1, "name": "square"}, {
            "id": 2, "name": "circle"}, {"id": 3, "name": "triangle"}]
        ...
        for img_id, d in enumerate(self.image_info):
            ...
            images.append(
                {"id": img_id, 'height': self.height, 'width': self.width})
            ...
            for (shape, color, dims) in d['shapes']:
                ...
                annotations.append({
                    ...
                    "image_id": int(img_id),
                    "area": float(area),
                    # note that the bboxes are in x, y , width, height format
                    "bbox": [float(bbox[0]), float(bbox[1]), float(bbox[2]) - float(bbox[0]) + 1, float(bbox[3]) - float(bbox[1]) + 1],
                    "iscrowd": 0
                })
...
def get_shapes_loader(batch_sz):
    ...
    train_loader = torch.utils.data.DataLoader(
        train_dt, batch_size=batch_sz, shuffle=True, num_workers=0, collate_fn=collate_fn)
    val_loader = torch.utils.data.DataLoader(
        val_dt, batch_size=batch_sz, shuffle=True, num_workers=0, collate_fn=collate_fn)
